...
def get_generic_list_filter(entity):
    class GenericListFilter(django_filters.FilterSet):
        def name_label_filter(self, queryset, name, value):
            """
            Filter for including the alternative names in the names search. The types of labels
            included in the query are currently hardcoded in a list.
        ...
        class Meta:
            model = ContentType.objects.get(
                app_label='entities', model=entity.lower()
            ).model_class()
